...
#####################
#Model structure
# This script trains and evaluates a Transformer for synonymous codon selection.
# The amino-acid sequence is fixed; the model chooses among synonymous DNA codons without changing the encoded protein.
# The main biological objectives are codon-usage fidelity, target GC control, tAI-like adaptation, and an MFE proxy.
# Later stages generate multiple candidate CDSs and save them for optional exact RNAfold evaluation.
#####################

# 1. INPUT
# Input 1 is the amino-acid sequence. Each amino acid becomes one model token.
# Input 2 is a global target GC fraction (for example, 0.43). This is conditioning information, not another sequence token.
# The model therefore learns: amino-acid sequence + desired GC level to synonymous codon choices.


# 2. EMBEDDING AND GC CONDITIONING
# Each amino-acid token is mapped to a learned dense vector. The scalar target GC is projected into the same d_model-dimensional space.
# The projected GC vector is added to every amino-acid embedding, allowing every position to know the global GC target while making its local codon decision.
# This lets local synonymous choices collectively move the final CDS toward the requested global GC content.


# 3. POSITIONAL ENCODING
# Self-attention alone does not encode sequence order. Sinusoidal positional encoding supplies information about where each amino acid occurs.
# The Transformer can therefore distinguish the same amino acid appearing at different positions and model position-dependent codon preferences.


# 4. TRANSFORMER ENCODER
# Four stacked Transformer encoder layers use multi-head self-attention to let positions interact with one another.
# This is useful because codon choices are not necessarily independent: the model can learn sequence-wide and long-range dependencies.
# Each encoder layer contains self-attention, a feed-forward network, and normalization.

# 5. OUTPUT PREDICTION
# The final linear layer converts each position's hidden representation into scores for the codon vocabulary.
# There are 64 possible DNA codons before biological masking. Softmax converts valid logits into a probability distribution.
# During decoding, the highest-scoring valid synonymous codon can be selected (argmax).


# 6. LOSS FUNCTION
# The training objective combines several signals rather than treating the observed codon as the only possible correct answer.
# Synonymous/soft-target loss learns codon usage from the reference data; GC loss pushes expected GC toward the requested target.
# Additional tAI-like and MFE-proxy losses can guide the generated sequence toward the corresponding target characteristics.
# The total loss is a weighted sum, so the lambda values determine how strongly each biological objective affects training.


# 7. OUTPUTS TRACKED DURING TRAINING
# The script stores total loss plus individual loss components and expected biological metrics for train and validation sets.
# Tracking the components separately makes it possible to see whether improvement comes from codon prediction, GC control, tAI-like control, or the MFE proxy.
#7. Outputs tracked:
#total loss
#cross-entropy loss
#GC loss
...
# After training, predictions are compared with the held-out test sequences.
# Evaluation includes codon accuracy, amino-acid fidelity, codon-usage similarity, GC difference, CAI, tAI-like difference, and MFE-proxy difference.
#8. Inference Behavior evaluation:
#accuracy
#codon distribution similarity
#GC content difference
#CAI

########HELPERS#########################
# 9. SEQUENCE PROCESSING HELPERS
# These functions validate and translate the biological input before it reaches the neural network.
# Their key job is to guarantee that the amino-acid and codon sequences are valid and positionally aligned.


#valid_aa(seq): Checks if the amino acid sequence is valid.
#Ensures every character is one of the 20 standard amino acids
#Removes corrupted or non-biological sequences.

#valid_dna(seq): Checks DNA validity.
#Ensures only A, T, C, G are present

#translate_dna(dna_seq): Converts DNA to amino acid sequence.
#Splits into codons
#Uses genetic code mapping
#Builds amino acid string


#*****All used to verify alignment between DNA and protein sequences.


# 10. ENCODING HELPERS
# Protein and codon strings are converted into integer token IDs so PyTorch can process them as tensors.
# Special tokens such as PAD/SOS/EOS are included where required by batching and sequence handling.



# 11. DATA ALIGNMENT HELPER
# Amino-acid input and codon target sequences use slightly different special-token conventions.
# This helper creates an aligned codon target so each amino-acid position corresponds to exactly one codon target position.



# 12. GC CONTENT HELPERS
# Each codon receives a GC score equal to the fraction of its three bases that are G or C.
# Example: GCG has GC=1.0, while ATG has GC=1/3. These per-codon values are used to calculate differentiable expected GC from model probabilities.
# Because expected GC is calculated before argmax decoding, GC loss can participate in backpropagation.
#build_gc_weight_vector(codon_vocab)
#Assigns GC value to each codon.
#Count G and C in each codon and Normalize:
#"GCG" to 1.0
#"ATG" to 0.33

#expected_gc_from_logits(logits, gc_weights)
#Computes expected GC content from probabilities
#Convert logits to probabilities (SoftMax)
#Weighted sum using GC weights


#compute_gc_loss(...)
#Calculates how far prediction is from target GC
#Compute expected GC per position
#Mask padding
#Average per sequence
#Compare with target GC
#Apply MSE loss



# 13. DECODING HELPERS
# These functions convert integer token IDs back into readable amino-acid and codon sequences for evaluation and output.



# 14. BASELINE HELPERS
# The most-frequent-codon baseline chooses the most commonly observed synonymous codon for each amino acid in the training data.
# This provides a simple reference point: the Transformer should be evaluated against this baseline rather than against zero performance.


#evaluate_most_frequent_codon_baseline()
#Tests baseline accuracy.


# 15. METRIC HELPERS
# These functions measure different aspects of sequence quality; no single metric captures all biological objectives.
# Accuracy measures exact codon agreement, cosine similarity measures overall codon-usage-pattern similarity, and GC measures global composition.
# CAI summarizes use of codons relative to training-derived preferred codons.


#codon_frequency_vector()
#Converts sequence → frequency vector

#cosine_similarity_vec()
#Compares codon usage patterns

#gc_content_from_codons()
#Calculates actual GC content of sequence

#build_codon_weights_from_training()
#Computes weights for CAI

#cai_of_codon_sequence()
...
# The following comments/links document the conceptual references used when designing the implementation.
...
#GC Content
#Perez, E., F. Strub, H. de Vries, V. Dumoulin, and A. Courville. 2018. FiLM: Visual reasoning with a general conditioning layer. Proc. AAAI Conf. Artif. Intell. 32.

#loss fucntion
#Song et al. (2016) — Training Deep Neural Networks via Direct Loss Minimization.
...
MAX_AA_LENGTH = 500
MAX_SAMPLES = 20000  #############uses all available filtered pairs if fewer than 50,000
BATCH_SIZE = 16
NUM_EPOCHS = 25
LEARNING_RATE = 0.0005
RANDOM_SEED = 1234

# Biological control settings
TARGET_GC = 0.43  # PlantForm-compatible target; close to NbT2T native CDS GC (~0.4241)
LAMBDA_GC = 3
...
# Decoding bonuses and post-generation candidate selection
GC_DECODING_BONUS = 1.55
...
#######
#Post-generation plant-expression motif filters.
#These are discrete sequence-design constraints, so they are applied after decoding, not as training losses.
...
#Candidate generation for later RNAfold.
#These candidates are generated without running RNAfold here.
...
print("Recommended: keep MAX_AA_LENGTH=400 for direct comparison/quick run; increase to 800 or 1200 only if memory allows.")


############################
#Genetic code and synonymous codons
...
def build_valid_codon_mask(aa_vocab, codon_vocab, aa_to_synonymous_codons, device):
    """valid_mask[aa_idx, codon_idx] = True if the codon can encode that amino acid."""
...
# Save the exact subset used for this training run
...
############################
#Encoding: generate numbers and vectors
#add special tokens
def encode_aa(seq):
...
# Each dataset item contains an amino-acid sequence, its reference codon sequence, and a global GC target.
# Variable-length proteins are padded into batches; padding positions are masked so they do not contribute to attention or losses.
## src: amino acids; tgt: codons; gc: global constraint

def collate_fn(batch):
    src_batch = [item[0] for item in batch]
    tgt_batch = [item[1] for item in batch]
    gc_batch = torch.stack([item[2] for item in batch])  # [B, 1]
...
# EMPIRICAL SYNONYMOUS-CODON DISTRIBUTION
# Estimate P(codon | amino acid) from the training data.
# This is useful because synonymous codons are biologically equivalent at the protein level, so the observed codon should not always be treated as the only valid target.
# The resulting distribution is used to build soft targets for the codon-prediction loss.
############################
# Synonymous codon soft target helpers

def build_empirical_codon_distribution(train_dataset, aa_vocab, codon_vocab, idx_to_aa, idx_to_codon):
    """Build q(codon | amino acid) from the training data."""
...
# MODEL COMPONENTS
# The following classes implement the learnable amino-acid embedding, GC conditioning, positional encoding, and Transformer encoder.
############################
#Position encode
...
############################
#ENCODER-ONLY: model adding GC content
...
#add gc in total loss; to every token as global constraint
        x = x + gc_embed
...
# DIFFERENTIABLE GC OBJECTIVE
# Expected GC is computed from the full predicted codon probability distribution rather than from a hard decoded sequence.
# This keeps the GC objective differentiable and allows gradients to change codon probabilities during training.
############################
#GC loss helpers
...
#Expected GC = weighted sum over codons; convert logit to possibilities
def expected_gc_from_logits(logits, gc_weights):
    """logits: [B, L, V]; gc_weights: [V]; returns expected GC per position: [B, L]"""
...
def compute_gc_loss(logits, aligned_target, gc_batch, gc_weights):
    expected_gc = expected_gc_from_logits(logits, gc_weights)  # [B, L]

    valid_mask = (aligned_target != codon_vocab["<PAD>"]).float()  # [B, L]

    gc_sum = (expected_gc * valid_mask).sum(dim=1)
    valid_count = valid_mask.sum(dim=1).clamp(min=1.0)
    avg_gc_per_seq = gc_sum / valid_count  # [B]

    target_gc = gc_batch.squeeze(1)        # [B]
...
# tAI-LIKE OBJECTIVE
# tRNA information is converted into codon weights. In this script the values are a tRNA gene-copy-based proxy, not experimentally measured tRNA abundance.
# The resulting expected tAI-like score can be compared with a target/reference score during training and evaluation.
############################
# tAI and MFE biological constraint helpers
...
def parse_trnascan_to_codon_counts(trna_file, genetic_code):
    """Parse tRNAscan-SE tabular output and convert anticodon gene counts to codon counts"""
    ...
    with f:
        for line in f:
            ...
            # tRNAscan-SE tabular columns are usually:
            # Name, tRNA#, Begin, End, Type, Codon/Anticodon, Intron Begin, Intron End, Score, Note...
            trna_type = parts[4]
            ...
            # Keep only anticodon/codon pairs that encode the reported isotype.
            # This excludes suppressors and inconsistent rows.
...
# For each amino acid, the codon with the highest matching tRNA gene count gets weight 1.0
#Other synonymous codons are scaled relative to that maximum
#Synonymous codons with no matching predicted tRNA gene get a small pseudocount-based weight, not exactly zero
...
# If no tRNA is detected for an amino acid, assign equal neutral weights.
        if max_count == 0:
            for codon in codons:
...
def compute_tai_loss(logits, aligned_target, tai_weights, target_tai=0.85, eps=1e-8):
    """Differentiable tAI-proxy loss"""
    probs = torch.softmax(logits, dim=-1)
    valid_mask = (aligned_target != codon_vocab["<PAD>"]).float()

# MFE PROXY
# This is deliberately a simplified proxy for RNA structural behavior, not an exact thermodynamic RNAfold MFE.
# The proxy uses nucleotide composition (GC/AU-related terms) because exact RNAfold is expensive and is not used inside the main differentiable training loop.
#######
#for each AA position
#compute probability over all codons
########
...
def build_codon_pairing_weight_vector(codon_vocab, device):
    """Build a simple differentiable MFE proxy weight"""
    weights = torch.zeros(len(codon_vocab), device=device)
    ...
    for codon, idx in codon_vocab.items():
        if codon in ["<PAD>", "<SOS>", "<EOS>"]:
            ...
            # GC pairs are stronger than AU pairs, so weight GC higher.
...
def tai_of_codon_sequence(codon_list, tai_weight_dict):
    """Sequence-level tAI-proxy score using geometric mean"""
    weights = []
...
def mfe_proxy_of_codon_sequence(codon_list):
    """Sequence-level MFE proxy score. This is not exact RNAfold MFE"""
    if len(codon_list) == 0:
        return 0.0
    scores = []
    for codon in codon_list:
...
#####Optional exact MFE evaluation using ViennaRNA RNAfold, if installed
...
def count_motif_hits(dna_sequence, motif_dict):
    """Count exact forbidden motif occurrences in a DNA sequence"""
    hits = {}
    total = 0
...
def motif_penalty_for_codons(codon_list):
    """Penalty for practical plant-expression instability motifs"""
    dna = "".join(codon_list)
...
#Score one candidate codon sequence for post-generation selection.
#Priority:
#1. Keep GC close to target.
#2. Prefer higher tRNA gene-copy-based tAI-like score.
...
# Candidate 1: deterministic greedy top-1
    greedy = []
...
# mixed target: alpha = 0 means normal hard CE;   alpha = 1 means fully empirical synonymous distribution
...
# entropy of predicted codon distribution
    pred_entropy = -(probs.clamp(min=eps) * torch.log(probs.clamp(min=eps))).sum(dim=-1)

# empirical entropy for the amino acid at each position
...
# TRAINING LOOP
# Each batch is passed through the Transformer, invalid synonymous codons are masked, and the individual loss components are calculated.
# The weighted total loss is backpropagated through the model and the optimizer updates the parameters.
############################
#Device
...
# VALIDATION LOOP
# Validation is performed without gradient updates. Its purpose is to monitor generalization and detect overfitting while training.
############################
...
# Build empirical synonymous codon distribution from training data
...
# Build differentiable tAI-like weights and MFE-proxy weights
if USE_TRNASCAN_TAI:
...
# Build a codon tAI weight dictionary for sequence-level summaries/selection.
...
# TEST / INFERENCE
# The held-out test set is evaluated only after training. Predictions are decoded into concrete synonymous codon sequences for biological metrics.
############################
#training_history
...
for epoch in range(NUM_EPOCHS):
    ...
    for src_batch, tgt_batch, gc_batch in train_loader:
        ...
        mask_for_batch = valid_codon_mask[src_batch]  # [B, L, V]
...
# PER-AMINO-ACID EVALUATION
# Accuracy is calculated separately for each amino acid because amino acids have different numbers of synonymous codons.
# Methionine and tryptophan have only one codon, whereas leucine, serine, and arginine have six; therefore exact codon accuracy has different difficulty across amino acids.
############################
#TEST LOSS + MODEL PREDICTIONS
...
# CODON-USAGE AND BIOLOGICAL METRICS
# The test stage calculates codon-frequency similarity, GC content, CAI, tAI-like score, and MFE proxy for reference, model, and baseline sequences.
############################
#BASELINE HELPERS
...
# FINAL REPORTING
# Print GC, tAI-like, MFE-proxy, amino-acid fidelity, codon accuracy, and the overall test-loss summary.
############################
#Print gc content
...
# POST-GENERATION CANDIDATE GENERATION
# Instead of keeping only one argmax sequence, generate several synonymous CDS candidates from the model's top codon choices.
# Candidates are ranked using GC closeness, tAI-like closeness, CAI, and motif penalties. Exact RNAfold is intentionally deferred to a later step.
############################
# Post-generation candidate generation for later RNAfold
...
# VISUALIZATIONS
# Plot training and validation curves to assess convergence and possible overfitting.
############################
#VISUALIZATIONS: loss curves
...
# Save the trained model, predictions, training history, and final metrics so the experiment can be reproduced or analyzed later.
#Save everything for further analysis
